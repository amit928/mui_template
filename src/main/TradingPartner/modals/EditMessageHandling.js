...
import { onUpdateMessageSpecialHandling } from "../action";
import { useMaterialUIController } from "context";

export default function EditMessageHandling({ open, handleClose, profileSetupList, currentMessageHandling, specialHandlingList }) {
    ...
    useEffect(() => {
        setis_active(currentMessageHandling.is_active);
        setconnectivity_profile_id(currentMessageHandling.tp_profile_id);
        setspecial_handling_type(currentMessageHandling.special_handling_type);
        setspecial_handling_name(currentMessageHandling.special_handling_name);
        setrule_description(currentMessageHandling.rule_description);
        setmodule_type(currentMessageHandling.module_type)

    }, [currentMessageHandling.id])
    ...
    useEffect(() => {
        specialHandlingList && specialHandlingList.results && specialHandlingList.results.length > 0 &&
            specialHandlingList.results.map((item, index) => {
                if (item.special_handling_name === special_handling_name) {
    ...
    const on_edit_message_handling = () => {
        if (connectivity_profile_id && special_handling_type && special_handling_name && rule_description) {
            ...
            dispatch(onUpdateMessageSpecialHandling(accessToken, body, currentMessageHandling.id));
            handleClose();
        }
        else {
        ...
        >
            <Paper style={{ backgroundColor: common_color.BACKGROUND_COLOR, color: common_color.TEXT_COLOR }}>
                ...
                <DialogContent>
                    ...
                    <MDBox component="form" role="form" >
                        ...
                        <MDBox mb={2} >
                            <Row>
                                <Col md="6">
                                    <MDSelect defaultValue={connectivity_profile_id} value={connectivity_profile_id} label="TP Profile Name" variant="outlined" onChange={(e) => { setconnectivity_profile_id(e.target.value) }} required={true} data={(profileSetupList.results && profileSetupList.results.length > 0) ?
                                        profileSetupList.results.map((item, index) => { return { label: item.profile_name, value: item.id } }) : []} />
                                </Col>

                                <Col md="6">
                                    <MDSelect defaultValue={special_handling_type} value={special_handling_type} label="Special Handling Type" variant="outlined" onChange={(e) => { setspecial_handling_type(e.target.value) }} required={true} data={[
                        ...
                        <MDBox mb={2} >
                            <Row>
                                <Col md="6">
                                    <MDSelect defaultValue={special_handling_name} value={special_handling_name} label="Special Handling Name" variant="outlined" onChange={(e) => { setspecial_handling_name(e.target.value) }} required={true} data={(specialHandlingList.results && specialHandlingList.results.length > 0) ?
                                        specialHandlingList.results.map((item, index) => { return { label: item.special_handling_name, value: item.id } }) : []} />
                                </Col>

                                <Col md="6">
                                    <TextField type="text" defaultValue={rule_description} value={rule_description} label="Rule Description" variant="outlined" required onChange={(e) => { setrule_description(e.target.value) }} fullWidth />
                                </Col>
                            </Row>
                        </MDBox>
                    </MDBox>
                ...
                <DialogActions>
                    ...
                    <MDButton style={{ width: 90 }} onClick={on_edit_message_handling} variant="gradient" color={sidenavColor} fullWidth>
                        Update
                    </MDButton>
                </DialogActions>
            </Paper>
