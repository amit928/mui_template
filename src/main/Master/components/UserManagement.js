...
import { deleteUser, getRoleList, getUserList } from "../action";
import DeleteModal from "../modals/DeleteModal";
import { PAGE_SIZE } from "lib/Constants";
import CopyUserDialog from "../modals/CopyUserDialog";
...
export default function UserManagement() {
    ...
    const userList = useSelector(store => store.master.userList)
    ...
    useEffect(() => {
        if (accessToken) {
            dispatch(getUserList(accessToken, { "page": 1, "page_size": PAGE_SIZE }))
                    ...
                    <Card>
                        ...
                        <MDBox pt={1}>
                            <Customize_DataTable
                                ...
                                onPageChange={(page) => { dispatch(getUserList(accessToken, { "page": page, "page_size": PAGE_SIZE })) }}
                                columns={[
                                    // { field: "id", headerName: "ID", width: 90 },
                                    {
                                        field: "action", headerName: "Action", width: 110, renderCell: (row) => <Typography>
                                            ...
                                            <Tooltip title="Copy" ><ContentCopyIcon fontSize="small" style={{ cursor: "pointer" }} onClick={() => { setopen_copy_modal(true); setcurrentUser(row.row) }} /></Tooltip>
                                        </Typography>
                                    },
                                    {
                                    ...
                                    {
                                        field: "name", headerName: "Name", width: 140, valueGetter: (params) =>
                                            `${params.row.first_name || ""} ${params.row.last_name || ""}`
                                    },
                                    { field: "organization_name", headerName: "Company Name", width: 130 },
                                    { field: "role_name", renderCell: (row) => <Typography style={{ fontSize: 15 }} >{row.row.role_data && row.row.role_data[0] && row.row.role_data[0].role_name}</Typography>, headerName: "Role Name", width: 140 },
                                    { field: "phone_number", headerName: "Phone Number", width: 140 },
                                    { field: "data_access", headerName: "Data Access", width: 130 },
