...
import { updateRole, updateUser } from "../action";
import { useMaterialUIController } from "context";

export default function EditRoleDialog({ open, handleClose, currentRole }) {
    ...
    const on_edit_role = () => {
        ...
        if (role_name && role_description && privilege_names) {
            dispatch(updateRole(accessToken, body, currentRole.id));
            handleClose();
        }
        else {
        ...
        >
            <Paper style={{ backgroundColor: common_color.BACKGROUND_COLOR, color: common_color.TEXT_COLOR }}>
                ...
                <DialogActions>
                    ...
                    <MDButton style={{ width: 90 }} onClick={on_edit_role} variant="gradient" color="dark" fullWidth>
                        update
                    </MDButton>
                </DialogActions>
            </Paper>
